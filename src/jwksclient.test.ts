...
test("fetch RSA key (which contains a kid) with kid", async () => {
  ...
  expect(
    await client.retrieve("https://test.com/jwks.json", "si_authentication"),
  ).toEqual(RSAPubKey1);
});
...
test("if kid is given fetching an RSA key (which doesn't contain a kid) should throw", async () => {
  ...
  await expect(
    client.retrieve("https://test.com/jwks.json", "si_authentication"),
...
test("search with kid", async () => {
  ...
  expect(await client.retrieve("https://test.com/jwks.json", "kid1")).toEqual(
    ECPubKey1,
  );
  expect(await client.retrieve("https://test.com/jwks.json", "kid2")).toEqual(
    RSAPubKey1,
  );
});
...
test("HTTPError if status !== 200", async () => {
  ...
  await expect(
    client.retrieve("https://test.com/jwks.json", "kid1"),
...
test("No kid or alg with multiple JWKs should throw", async () => {
  ...
  await expect(client.retrieve("https://test.com/jwks.json")).rejects.toThrow(
    Error,
  );
});
...
test("Invalid kid should throw", async () => {
  ...
  await expect(
    client.retrieve("https://test.com/jwks.json", "error"),
...
test("Invalid JWKs should throw", async () => {
  ...
  await expect(
    client.retrieve("https://test.com/jwks.json", "test"),
  ).rejects.toThrow(Error);
});
...
const ECPubJWK1 = JSON.parse(
  readFileSync("src/fixtures/JWKs/ECPubJWK1.json", "utf8"),
);
const ECPubJWKId1 = JSON.parse(
  readFileSync("src/fixtures/JWKs/ECPubJWKId1.json", "utf8"),
);
const RSPubJWK1 = JSON.parse(
  readFileSync("src/fixtures/JWKs/RSPubJWK1.json", "utf8"),
);
const RSPubJWKId1 = JSON.parse(
  readFileSync("src/fixtures/JWKs/RSPubJWKId1.json", "utf8"),
);
const MixPubJWKId1 = JSON.parse(
  readFileSync("src/fixtures/JWKs/MixPubJWKId1.json", "utf8"),
);
const SymmetricJWK1 = JSON.parse(
  readFileSync("src/fixtures/JWKs/SymmetricJWK1.json", "utf8"),
);
const RepeatedJWK1 = JSON.parse(
  readFileSync("src/fixtures/JWKs/RepeatedPubJWKId1.json", "utf8"),
);
